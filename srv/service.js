module.exports = cds.service.impl(async function(){
    ...
    if (
        parseFloat(request.data.PRICE) >= 1500 &&
        request.data.CURRENCY_CODE === 'EUR'
    ) {
        request.error(
            500,
            "Please get the approval from your product manager."
        );
    }
    if (
        parseFloat(request.data.PRICE) >= 2000 &&
        request.data.CURRENCY_CODE === 'USD'
    ) {
        request.error(
            500,
            "Please get the approval from your product manager."
...
// on : Run where the generic handler runs.
...
this.on('getTop10Products', async (request, response) => {
    try {
        ...
        // Step-2 : Get top 10 products using Transaction object
...
this.on('createEmployee', async (request, response) => {
    ...
    const returnData = await transaction.run([
        ...
        if (typeof(resolve) !== undefined) {
            ...
            request.error(
                500,
                "Error in creating the employee information"
            );
...
this.on('discountPrice', async (request, response) => {
    ...
    try {
        ...
        // (discount on Gross Amount, Net Amount, Tax Amount)
        ...
        const podata = await transaction.read(POSrv).where(ID);
...
this.on('largestOrder', async (request, response) => {
    ...
    try {
        ...
        // Step-2 : Read the purchase order based on gross amount
